...
```{r}
fst_fis = lapply(levels(pop(genind)), function(x) {
	inds = pop(genind) == x
	wc(genind[inds])
})

fstats = lapply(levels(pop(genind)), function(x) {
	inds = pop(genind) == x
	fst = Fst(genind[inds] %>% as.loci)
	colMeans(fst)
})

names(fst_fis) = names(fstats) = levels(pop(genind))
...
table.value(
	table(new_popmap$Country, clusters_karite$grp),
